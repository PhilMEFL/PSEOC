<#
.SYNOPSIS
	Collects information about McAfee versions and populates the McAfee_DashBoard_KPI.xlsx sheet
...
#	$strWorkBook = "{0}\Data\2018_12_19_McAfee53x_SIP.csv" -f $rootscriptDir
	...
	$hshResult['EndPoint¥Compliant¥(3477 & 3480)'] = ($arrCsv.'AMCore Content Version' | ?{$_ -in (($arrCsv.'AMCore Content Version') | sort -Unique | select -Last $intLastRec | ?{$_ –ne ''})}).count
	...
	$arrValues = ($arrCsv.($hshHeader.DAT)) | sort -Unique  | ?{$_ –ne ''}
	
	$arrCsv.($hshHeader.DAT) | ?{$_ -in ($arrValues) | select -Last $intLastRec | ?{$_ –ne ''}}.count
	$hshResult['VirusScan¥Compliant ¥(8963 & 8968)'] = ($arrCsv.($hshHeader.DAT) | ?{$_ -in ($arrValues)} ).count

	# get the number of non compliant VirusScan computers
	$hshResult['VirusScan¥Non Compliant'] = ($arrCsv.($hshHeader.DAT) | ?{!($_ -in ($arrValues)) -and ($_ -ne '')} ).count
